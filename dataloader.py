...
#purpose of this class is to prevent mistakes inserting wrongly built dataframes
class Path_labels:
    #This is just a wrapper enclosing a DataFrame to specify its required structure
    columns=['ClipID', 'Boredom', 'Engagement', 'Confusion', 'Frustration', 'f_name', 'path']
    #Warning: 'Frustration' can be generated as 'Frustration ' <- see the blank space in its end
    def __init__(self, df_labels:pd.DataFrame):
        assert (np.array(df_labels.columns)==np.array(Path_labels.columns)).all()
        self.__df_labels=df_labels
    def __call__(self):
        return self.__df_labels

#This handles openface csv files
class OpenFace_DataLoader:
    columns=['frame',
            'face_id',
            'timestamp',
            'confidence',
    ...
    def __init__(self, 
                 n, #number of frames you would like to take from each video
                 path_labels:Path_labels, #meta data object including data path and label information. Defined above
                 batch_size, #size of each batch
                 feature_set=None, #choice of columns from the whole data
                 seed=0, #seed for reproductivity
                 x_col=None,
                 y_col=None,
                 **kwargs):
        #argument conditions
        self.__seed=seed
        if not batch_size%n==0:
            raise AssertionError('batchsize must be able to devided by n')
        self.__n=n
        self.batch_size=batch_size
        self.debug=True if 'debug' in kwargs and kwargs['debug']==True else False
        if not isinstance(feature_set, type(None)):
            for feature in feature_set:
                # if feature not in OpenFace_DataLoader.columns:
                assert feature.replace(' ', '') in OpenFace_DataLoader.columns
            self.feature_set=feature_set
            
            #make x_col, y_col from feature_set
            if isinstance(x_col, type(None)):
                x_col=[x for x in feature_set if x not in [\
            ...
            for _x_col in x_col:
                assert _x_col in feature_set
            for _y_col in y_col:
                assert _y_col in feature_set
        else:
            self.feature_set=-1
            
            #make x_col, y_col from everything
            if isinstance(x_col, type(None)):
                x_col=[x for x in OpenFace_DataLoader.columns if x not in [\
                    'frame', 'face_id', 'timestamp', 'confidence', 'success',
                    'Boredom', 'Engagement', 'Confusion', 'Frustration']]
            if isinstance(y_col, type(None)):
                y_col=['Boredom', 'Engagement', 'Confusion', 'Frustration']
        
        x_col_idx=[list(OpenFace_DataLoader.columns).index(col) for col in x_col]
        y_col_idx=[list(OpenFace_DataLoader.columns).index(col) for col in y_col]
        
        if self.debug:
            print(path_labels().columns)

        label_iter=iter(path_labels().values)
        
        np.random.seed(self.__seed)
        self.len_data=self.__n*len(path_labels().index) #total length = number of frames * number of videos(.csv) to read
        if self.debug:
            print(self.len_data)
        
        self.batch_list=[]
        #load dataframe that label_iter points (label_iter is a linked list having file destination to read),
        # samples some frames (with number of n),
        # and merged the samples with label information
        def build_sample(n, label_iter): 
            labels=next(label_iter)
            df_sample=pd.read_csv(labels[-1], sep='[,][ ]+', engine='python').sample(n=n, frac=None, random_state=None) #use global seed
            ...
            if self.feature_set!=-1:
                return df_sample.loc[:, self.feature_set]
                # return df_sample.loc[:, x_col], df_sample.loc[:, y_col]
            else:
                return df_sample
            
        for batch in range(self.len_data//self.batch_size+1):
            idx_start=batch*self.batch_size
            idx_end=(batch+1)*self.batch_size if (batch+1)*self.batch_size<self.len_data else self.len_data
            if self.debug:
            ...
            if self.debug:
                print(sample.shape)
            self.batch_list.append((sample[:, x_col_idx], sample[:, y_col_idx].long()))

    def __setitem__(self, index, labels:Path_labels, mode='replace'):
        #haven't tested code
        #method to add new element in self.batch_list
        assert mode in ['replace', 'stack']
        df_sample=pd.read_csv(labels()[-1], sep='[,][ ]+', engine='python').sample(n=self.__n, frac=None, random_state=None) #use global seed
        df_sample['Boredom']=labels()[1] #add Boredom column
        df_sample['Engagement']=labels()[2] #add Engagement column
        df_sample['Confusion']=labels()[3] #add Confusion column
        df_sample['Frustration']=labels()[4] #add Frustration column
        if self.feature_set!=-1:
            df_sample=df_sample.loc[:, self.feature_set]
        if mode=='replace':
            raise NotImplementedError
            self.batch_list[index]=torch.Tensor(df_sample.values)
        elif mode=='stack':
            raise NotImplementedError
            self.batch_list[index]=torch.vstack((self.batch_list[index], 
                                                 torch.Tensor(df_sample.values)))
    ...
    def to(self, device:str) -> list:
        if device=='numpy':
            for i in range(len(self.batch_list)):
                x, y=self.batch_list[i]
                self.batch_list[i]=x.detach().cpu().numpy(), y.detach().cpu().numpy()
        else:
            for i in range(len(self.batch_list)):
                x, y=self.batch_list[i]
                self.batch_list[i]=x.to(device), y.to(device)
